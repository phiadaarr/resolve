...
def main():
    ...
    parser.add_argument("--pe", help="Parallel environment, e.g. 'mpi-20'",
        help="On hilbert allowed values are 'sm mpi mpi-5 mpi-10 mpi-20 mpi-40 mpi-128 mpi-256'. "
             "The number specifies how many threads shall be run per node.", default="sm")
    parser.add_argument("--total-threads", type=int, default=1, help="Total number of threads")
    parser.add_argument("--np", type=int, default=1, help="Number of tasks. Only present for mpi environments")
    ...
    for iteration in range(total_iterations):
        ...
        jobname = f'{job_name_prefix}{iteration}{timestamp}'

        # Assemble main resolve call
        main_call = ""
        if "mpi" in args.pe:
            assert args.np % args.total_number_threads == 0
            j = args.np // args.total_number_threads
            ...
            j = args.total_number_threads
        else:
            raise ValueError(f"--pe {args.pe} not known")

        main_call += "resolve -j{j} --resume --terminate {iteration} {cfg_file} "
...
#$ -l qname={args.qname}
#$ -cwd
#$ -pe {args.pe} {total_number_threads}
#$ -N {jobname}
#$ -hold_jid {previous_jobname}

setenv MPLBACKEND agg
source venv{qname}/bin/activate

setenv OMP_NUM_THREADS {args.nthreads}
