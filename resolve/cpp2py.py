...
class Pybind11LikelihoodEnergyOperator(ift.LikelihoodEnergyOperator):
    def __init__(self, dom, op, draw_sample, get_transformation,
                 data_residual=None, sqrt_data_metric=None):
        self._domain = ift.makeDomain(dom)
        self._op = op
        self._draw_sample = draw_sample
        self.get_transformation = get_transformation
        super(Pybind11LikelihoodEnergyOperator, self).__init__(data_residual, sqrt_data_metric)
    ...
    def apply(self, x):
        ...
        res = x.new(pos, jac)
        if x.want_metric:
            metric = Pybind11SelfAdjointOperator(self.domain, lin.apply_metric,
                                                 self._draw_sample(x.val.val))
