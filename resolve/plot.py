...
def _register_cmaps():
    ...
    import matplotlib.pyplot as plt
    planckcmap = {
        'red': ((0., 0., 0.), (.4, 0., 0.), (.5, 1., 1.), (.7, 1., 1.),
                (.8, .83, .83), (.9, .67, .67), (1., .5, .5)),
        'green': ((0., 0., 0.), (.2, 0., 0.), (.3, .3, .3), (.4, .7,
                                                             .7), (.5, 1., 1.),
                  (.6, .7, .7), (.7, .3, .3), (.8, 0., 0.), (1., 0., 0.)),
        'blue': ((0., .5, .5), (.1, .67, .67), (.2, .83, .83), (.3, 1., 1.),
                 (.5, 1., 1.), (.6, 0., 0.), (1., 0., 0.))
    }
    he_cmap = {
        'red': ((0., 0., 0.), (.167, 0., 0.), (.333, .5, .5), (.5, 1., 1.),
                (1., 1., 1.)),
        'green': ((0., 0., 0.), (.5, 0., 0.), (.667, .5, .5), (.833, 1., 1.),
                  (1., 1., 1.)),
        'blue': ((0., 0., 0.), (.167, 1., 1.), (.333, .5, .5), (.5, 0., 0.),
                 (1., 1., 1.))
    }
    fd_cmap = {
        'red': ((0., .35, .35), (.1, .4, .4), (.2, .25, .25), (.41, .47, .47),
                (.5, .8, .8), (.56, .96, .96), (.59, 1., 1.), (.74, .8, .8),
                (.8, .8, .8), (.9, .5, .5), (1., .4, .4)),
        'green': ((0., 0., 0.), (.2, 0., 0.), (.362, .88, .88), (.5, 1., 1.),
                  (.638, .88, .88), (.8, .25, .25), (.9, .3, .3), (1., .2,
                                                                   .2)),
        'blue': ((0., .35, .35), (.1, .4, .4), (.2, .8, .8), (.26, .8, .8),
                 (.41, 1., 1.), (.44, .96, .96), (.5, .8, .8), (.59, .47, .47),
                 (.8, 0., 0.), (1., 0., 0.))
    }
    fdu_cmap = {
        'red': ((0., 1., 1.), (0.1, .8, .8), (.2, .65, .65), (.41, .6, .6),
                (.5, .7, .7), (.56, .96, .96), (.59, 1., 1.), (.74, .8, .8),
                (.8, .8, .8), (.9, .5, .5), (1., .4, .4)),
        'green': ((0., .9, .9), (.362, .95, .95), (.5, 1., 1.),
                  (.638, .88, .88), (.8, .25, .25), (.9, .3, .3), (1., .2,
                                                                   .2)),
        'blue': ((0., 1., 1.), (.1, .8, .8), (.2, 1., 1.), (.41, 1., 1.),
                 (.44, .96, .96), (.5, .7, .7), (.59, .42, .42), (.8, 0., 0.),
                 (1., 0., 0.))
    }
    pm_cmap = {
        'red': ((0., 1., 1.), (.1, .96, .96), (.2, .84, .84), (.3, .64, .64),
                (.4, .36, .36), (.5, 0., 0.), (1., 0., 0.)),
        'green': ((0., .5, .5), (.1, .32, .32), (.2, .18, .18), (.3, .8, .8),
                  (.4, .2, .2), (.5, 0., 0.), (.6, .2, .2), (.7, .8, .8),
                  (.8, .18, .18), (.9, .32, .32), (1., .5, .5)),
        'blue': ((0., 0., 0.), (.5, 0., 0.), (.6, .36, .36), (.7, .64, .64),
                 (.8, .84, .84), (.9, .96, .96), (1., 1., 1.))
    }
    ...
    plt.register_cmap(
        cmap=LinearSegmentedColormap("Faraday Uncertainty", fdu_cmap))
    plt.register_cmap(cmap=LinearSegmentedColormap("Plus Minus", pm_cmap))


def _plot0D(f, ax, **kwargs):
    import matplotlib.pyplot as plt
    if len(f) > 1:
        raise ValueError
    f = f[0]
    ...
    ax.bar(np.arange(f.domain.size), f.to_global_data())
    _limit_xy(**kwargs)
...
def _plothist(f, ax, **kwargs):
    ...
    for ff in f:
        ax.hist(ff.to_global_data().flatten(), bins=kwargs.pop('bins', 200))
...
def _plot1D(f, ax, **kwargs):
    ...
    for i, fld in enumerate(f):
        if not isinstance(fld, ift.Field):
    ...
    if isinstance(dom, ift.RGSpace):
        ...
        if label != ([None]*len(f)):
            plt.legend()
        return
    elif isinstance(dom, ift.LogRGSpace):
        npoints = dom.shape[0]
        ...
        if label != ([None]*len(f)):
            plt.legend()
        return
    elif isinstance(dom, ift.PowerSpace):
...
def _plot2D(f, ax, **kwargs):
    ...
    if len(dom.shape) == 2 and all(
        [isinstance(dd, ift.UnstructuredDomain)
         for dd in dom]) and dom.shape[1] in [2, 3]:
        fld = f.to_global_data().T
        dct = {'x': fld[0], 'y': fld[1]}
        if dom.shape[1] == 3:
            dct['c'] = fld[2]
        im = ax.scatter(**dct, s=1., **norm, cmap=cmap)
        plt.colorbar(im)
        _limit_xy(**kwargs)
        return
    if isinstance(dom[0], ift.UnstructuredDomain) and isinstance(
            dom[1], ift.RGSpace):
        vol = dom[1].shape[0]*dom[1].distances[0]
        newdom = ift.RGSpace(
            dom.shape[::-1],
            distances=[dom[1].distances[0], vol/dom.shape[0]/2])
        newdom = ift.RGSpace(
            dom.shape[::-1], distances=[dom[1].distances[0], 1.])
        aspect = {'aspect': 'auto'}
        f = ift.from_global_data(newdom, f.to_global_data().T)
        dom = f.domain
    dom = dom[0]
    if isinstance(dom, ift.RGSpace):
        nx, ny = dom.shape
        ...
        _limit_xy(**kwargs)
        return
    elif isinstance(dom, (ift.HPSpace, ift.GLSpace)):
        import pyHealpix
        xsize = 800
        res, mask, theta, phi = _mollweide_helper(xsize)
        if isinstance(dom, ift.HPSpace):
...
def _plot(f, ax, **kwargs):
    _register_cmaps()
    if isinstance(f, ift.Field):
        f = [f]
    f = list(f)
    if len(f) == 0:
        raise ValueError("need something to plot")
    if not isinstance(f[0], ift.Field):
        raise TypeError("incorrect data type")
    dom1 = f[0].domain
    if (len(dom1) == 1 and (isinstance(dom1[0], ift.PowerSpace) or
                            (isinstance(dom1[0], (ift.RGSpace, ift.LogRGSpace))
                             and len(dom1[0].shape) == 1))):
        _plot1D(f, ax, **kwargs)
        return
    elif len(dom1) == 1 and isinstance(dom1[0], ift.UnstructuredDomain):
        _plot0D(f, ax, **kwargs)
        return
    elif len(dom1.shape) == 2:
        if len(f) != 1:
            raise ValueError("need exactly one Field for 2D plot")
        _plot2D(f[0], ax, **kwargs)
        return
    elif all([isinstance(dd, ift.UnstructuredDomain) for dd in dom1]):
    ...
    def output(self, **kwargs):
        ...
        if nx*ny < nplot:
            raise ValueError(
                ...
                'Available plot slots: {}, number of plots: {}'.format(
                    nx*ny, nplot))
